...
export class SLTP extends Backtest {
    inloop({ i }: { i: number }): void {
        console.log("SLTP: inloop");
        const _row = this.row;
        ...
        if (!this.pos && this.buyCond(this.prevrow, this.df, i)) {
            ...
            if (
                ...
                this.entryLimit = this.row.o // Math.max(this.prevrow.o, this.prevrow.c); //o;
            ...
            this.enterTs = this.row.ts;
            this.exitLimit = this.row.o * (1 + .5/100)
        }
